...
QString QtDoublePropertyManager::valueText(const QtProperty *property) const
{
    ...
    // Use default constructor to use locale defined as default locale
    QLocale myLocale;

    // Taken from QString QDoubleSpinBox::textFromValue(double value) const
    QString str = myLocale.toString(it.value().val, 'f', it.value().decimals);
    if (qAbs(it.value().val) >= 1000.0) {
        str.remove(myLocale.groupSeparator());
    }
