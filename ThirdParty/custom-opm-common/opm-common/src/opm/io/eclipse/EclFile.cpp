...
namespace EclIO
{
    ...
    void EclFile::loadFormattedArray(const std::string& fileStr, std::size_t arrIndex, int64_t fromPos)
    {
        ...
        case C0NN:
            char_array[arrIndex]
                = readFormattedCharArray(fileStr, array_size[arrIndex], fromPos, array_element_size[arrIndex]);
            break;
        case MESS:
            break;
        default:
        ...
        if (formatted) {
            ...
            for (unsigned int arrIndex = 0; arrIndex < array_name.size(); arrIndex++) {
                ...
                if (array_name[arrIndex] == name) {
                    ...
                    size_t size
                        = sizeOnDiskFormatted(array_size[arrIndex], array_type[arrIndex], array_element_size[arrIndex])
                        + 1;
    ...
    void EclFile::loadData(int arrIndex)
    {
        if (formatted) {
            ...
            size_t size
                = sizeOnDiskFormatted(array_size[arrIndex], array_type[arrIndex], array_element_size[arrIndex]) + 1;
            std::vector<char> buffer(size);
            inFile.read(buffer.data(), size);
    ...
    template <>
    const std::vector<float>& EclFile::get<float>(int arrIndex)
    {
    ...
    std::streampos EclFile::seekPosition(const std::vector<std::string>::size_type arrIndex) const
    {
        ...
        const auto seekpos = (datapos <= headerSize) ? 0ul : datapos - headerSize;
    ...
    const std::vector<std::string>& EclFile::get<std::string>(const std::string& name)
    {
        ...
        if ((array_type[search->second] != Opm::EclIO::C0NN) && (array_type[search->second] != Opm::EclIO::CHAR)) {
            std::string message
                = "Array with index " + std::to_string(search->second) + " is not of type " + "std::string";
            OPM_THROW(std::runtime_error, message);
        }
    ...
    template <class T>
    const std::vector<T>& EclFile::getImpl(int arrIndex,
                                           eclArrType type,
