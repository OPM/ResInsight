...
# endif
      ...
      public:
        ...
        typedef const string_type & reference;
        typedef ptrdiff_t           difference_type;
        typedef const string_type * pointer;
...
# if !(defined(_MSC_VER) && _MSC_VER <= 1300) // && !BOOST_WORKAROUND(__BORLANDC__, < 0x610)
    template< class Path >
...
# elif 0 // BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x564))
    template< class String, class Traits >
...
# endif
    ...
    public:
      ...
      ~basic_filesystem_error() throw() { delete m_imp_ptr; }

      const path_type & path1() const
      {
        static const path_type empty_path;
        return m_imp_ptr != 0 ? m_imp_ptr->m_path1 : empty_path ;
      }
      const path_type & path2() const
      {
        static const path_type empty_path;
        return m_imp_ptr != 0 ? m_imp_ptr->m_path2 : empty_path ;
      }
    ...
    private:
      ...
      // std::tr1::shared_ptr<m_imp> m_imp_ptr;
      m_imp* m_imp_ptr;
    };
...
#       if !(defined(_MSC_VER) &&  _MSC_VER <= 1310)  // VC++ 7.1
