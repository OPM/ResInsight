...
# endif // ifndef BOOST_FILESYSTEM_NARROW_ONLY
  ...
  get_current_path_template( String & ph )
  {
    ...
    value_type* buf = new value_type[sz];
  ...
  get_full_path_name_template( const String & ph, String & target )
  {
    ...
    if ( len > buf_size )
    {
      ...
      value_type* big_buf = new value_type[len];
      if ( (len=get_full_path_name( ph, len , big_buf/*.get()*/, &pfn )) == 0 ) {
        delete [] big_buf;
        return error_code( ::GetLastError(), system_category );
      }
      big_buf[len] = '\0';
      target = big_buf; // big_buf.get();
      delete [] big_buf;
      return ok;
    }
    buf[len] = '\0';
...
#   else // BOOST_POSIX_API
      ...
      get_current_path_api( std::string & ph )
      {
        for ( long path_max = 32;; path_max *=2 ) // loop 'til buffer large enough
        {
          ...
          char* buf = new char[static_cast<std::size_t>(path_max)];
          if ( ::getcwd( buf, static_cast<std::size_t>(path_max) ) == 0 )
          {
            if ( errno != ERANGE
