...
# endif
      ...
      const Path & base/* = initial_path<Path>() */)
    {
      assert( base.is_complete()
        && (ph.is_complete() || !ph.has_root_name()) ); // "boost::filesystem::complete() precondition not met"
...
//  basic_directory_iterator  ------------------------------------------------//
    ...
    public:
      ...
      const basic_directory_iterator& operator++() {
        increment();
        return *this;
      }
      ...
      bool operator==(const basic_directory_iterator& rhs) const
      { return equal (rhs); }
    ...
    private:
      ...
      // std::tr1::shared_ptr< detail::dir_itr_imp< Path > >  m_imp;
      detail::dir_itr_imp< Path >* m_imp;
...
# endif
      ...
      const Path & dir_path )
    {
      ...
      if ( m_imp->m_handle == 0 ) {
        delete m_imp;
        m_imp = 0; // eof, so make end iterator
      }
      else // not eof
      {
    ...
    {
      assert( m_imp != 0 ); // attempt to increment end iterator
      assert( m_imp->m_handle != 0 ); // internal program error
      ...
      for (;;)
      {
        ...
        if ( m_imp->m_handle == 0 ) { delete m_imp; m_imp = 0; return; } // eof, make end
        if ( !(name[0] == dot<Path>::value // !(dot or dot-dot)
          && (name.size() == 1
            || (name[1] == dot<Path>::value
              && name.size() == 2))) )
