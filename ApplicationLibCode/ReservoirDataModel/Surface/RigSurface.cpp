...
{
    m_vertexResults[resultName] = resultValues;
}
...
{
    auto it = m_vertexResults.find( propertyName );
    if ( it != m_vertexResults.end() )
    {
        return it->second;
    }
...
std::vector<QString> RigSurface::propertyNames() const
{
    ...
    for ( const auto& propertyResult : m_vertexResults )
    {
