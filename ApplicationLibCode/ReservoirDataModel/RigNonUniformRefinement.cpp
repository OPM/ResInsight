...
///
//--------------------------------------------------------------------------------------------------
RigNonUniformRefinement::RigNonUniformRefinement( size_t sectorSizeI, size_t sectorSizeJ, size_t sectorSizeK )
{
    m_sectorSize[0] = sectorSizeI;
    m_sectorSize[1] = sectorSizeJ;
    m_sectorSize[2] = sectorSizeK;
                                                           ...
                                                           const std::vector<double>& widths )
{
    ...
    for ( size_t c = 0; c < numCells; ++c )
    {
        ...
        setCumulativeFractions( dim, startIndex + c, localFractions );
    }
}
...
RigNonUniformRefinement
    RigNonUniformRefinement::fromUniform( const cvf::Vec3st& refinement, size_t sectorSizeI, size_t sectorSizeJ, size_t sectorSizeK )
{
    RigNonUniformRefinement result( sectorSizeI, sectorSizeJ, sectorSizeK );
    ...
    for ( auto dim : { DimI, DimJ, DimK } )
    {
        ...
        // Create uniform cumulative fractions
        std::vector<double> uniformFractions;
        uniformFractions.reserve( refCount[dim] );
        for ( size_t i = 1; i <= refCount[dim]; ++i )
        {
            uniformFractions.push_back( static_cast<double>( i ) / static_cast<double>( refCount[dim] ) );
        }
        uniformFractions.back() = 1.0;
