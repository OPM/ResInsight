...
std::set<int> RigVisibleCategoriesCalculator::visibleCategories( RimEclipseView* eclView )
{
...

    {
        ...
        std::set<size_t> visibleReservoirCells =
            RigVisibleCategoriesCalculator::visibleFaultAndIntersectionCells( eclView );
...
///
//--------------------------------------------------------------------------------------------------
std::set<size_t> RigVisibleCategoriesCalculator::visibleFaultAndIntersectionCells( RimEclipseView* eclView )
{
    std::set<size_t> visibleReservoirCells;
             ...
             !eclView->faultResultSettings()->showCustomFaultResult() )
        {
            for ( const auto& f : eclView->faultCollection()->faults() )
            {
                if ( f->showFault() )
                {
                    ...
                    {
                        visibleReservoirCells.insert( faultFace.m_nativeReservoirCellIndex );
                        visibleReservoirCells.insert( faultFace.m_oppositeReservoirCellIndex );
                    }
                }
            }
        }
...

    {
        ...
        if ( eclView->separateSurfaceResultsCollection()->isActive() )
        {
            ...
            if ( eclView->surfaceInViewCollection() && eclView->surfaceInViewCollection()->isChecked() )
            {
                ...
                intersectionGeoGenerators.insert( intersectionGeoGenerators.end(),
                                                  geoGenerators.begin(),
                                                  geoGenerators.end() );
            }
        }
        ...
        for ( const auto geoGenerator : intersectionGeoGenerators )
        {
            ...
            {
                visibleReservoirCells.insert( cIdx );
            }
        }
    }

    return visibleReservoirCells;
}
