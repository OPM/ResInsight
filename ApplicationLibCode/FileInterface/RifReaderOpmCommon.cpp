...
std::int64_t RifReaderOpmCommon::restartDataSize( const Opm::EclIO::ERst& restartFile, const std::string& keyword, int reportStepNumber )
{
    ...
    // SEQNUM marks the start of each report step in a unified restart file. Separate restart files contain one report
    // step and no SEQNUM. Start at -1 so the first SEQNUM advances the unified report index to zero.
