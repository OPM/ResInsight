    ...
    while ( true )
    {
        ...
        if ( stream )
        {
            ...
            else if ( uppercasedLine.starts_with( "*ELEMENT," ) )
            {
                ...
                readElements( stream, partElements );
...
///
//--------------------------------------------------------------------------------------------------
void RifInpReader::readNodes( std::istream& stream, std::vector<std::pair<int, cvf::Vec3d>>& nodes )
{
...
void RifInpReader::readElements( std::istream& stream, std::vector<std::pair<int, std::vector<int>>>& partElements )
{
