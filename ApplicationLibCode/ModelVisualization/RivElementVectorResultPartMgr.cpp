...
void RivElementVectorResultPartMgr::appendDynamicGeometryPartsToModel( cvf::ModelBasicList* model, size_t timeStepIndex )
{
    ...
    if ( result->showNncData() )
    {
        ...
        nncData->buildPolygonsForNativeConnections();
        ...
        for ( size_t nIdx = 0; nIdx < nncData->nativeConnectionCount(); ++nIdx )
        {
            const RigConnection& conn = nncData->nativeConnections()[nIdx];
