...
///
//--------------------------------------------------------------------------------------------------
QColor RimEnsembleCurveSet::colorForLegend() const
{
...
std::vector<cvf::Color3f> RimEnsembleCurveSet::generateColorsForCases( const std::vector<RimSummaryCase*>& summaryCases ) const
{
    ...
    else if ( RimEnsembleCurveSetColorManager::hasSameColorForAllCurves( m_colorMode() ) )
    {
...
void RimEnsembleCurveSet::updateCurveColors()
{
    ...
    if ( m_plotCurveForLegendText )
    {
        m_plotCurveForLegendText->setColor( colorForLegend() );
    }
    ...
    if ( plot && plot->plotWidget() )
    {
        if ( m_yValuesSummaryCaseCollection() && isCurvesVisible() &&
             !RimEnsembleCurveSetColorManager::hasSameColorForAllCurves( m_colorMode() ) && m_legendConfig->showLegend() )
        {
            if ( !m_legendOverlayFrame )
            {
...
void RimEnsembleCurveSet::updateEnsembleLegendItem()
{
...

    {
        ...
        if ( RimEnsembleCurveSetColorManager::hasSameColorForAllCurves( m_colorMode() ) )
        {
            QColor curveColor = colorForLegend();
