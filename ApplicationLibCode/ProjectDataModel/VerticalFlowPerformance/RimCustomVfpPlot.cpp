...
RimCustomVfpPlot::RimCustomVfpPlot()
{
    ...
    CAF_PDM_InitFieldNoDefault( &m_compareTables, "CompareTables", "Compare Tables" );
    m_compareTables.uiCapability()->setUiEditorTypeName( caf::PdmUiTreeSelectionEditor::uiEditorTypeName() );
    ...
    m_colorLegend->legendChanged.connect( this, &RimCustomVfpPlot::legendColorsChanged );
...
void RimCustomVfpPlot::selectDataSource( RimVfpTable* mainDataSource, const std::vector<RimVfpTable*>& vfpTableData )
{
    ...
    m_compareTables.setValue( vfpTableData );
}
...
{
    auto colors = RiaColorTables::faultsPaletteColors();

    // Select a subset of colors that can be used as individual curve set colors. Make sure the colors are distinct.
    int colorIndex = 1;
    for ( int i : { 1, 2, 3, 6, 9, 4 } )
    {
        auto  color           = colors.cycledColor3f( i );
...
QString RimCustomVfpPlot::asciiDataForPlotExport() const
{
    ...
    for ( const auto& curveData : m_plotData )
    {
        for ( size_t curveIdx = 0; curveIdx < curveData.size(); curveIdx++ )
        {
            ...
            if ( !m_compareTables.empty() && plotCurveIdx < m_plotCurveMetaData.size() )
            {
...
void RimCustomVfpPlot::onLoadDataAndUpdate()
{
    ...
    auto compareTables = m_compareTables.ptrReferencedObjectsByType();
    std::copy( compareTables.begin(), compareTables.end(), std::back_inserter( tables ) );
    ...
    if ( curveSetCount > 1 )
    {
        ...
        for ( size_t i = 0; i < curveSetCount; i++ )
        {
            ...
            if ( plotCurveIdx < m_plotCurveMetaData.size() )
            {
                ...
                categories.push_back( std::make_tuple( curveSetName, i, cvf::Color3ub( color ) ) );
            }
...
std::vector<double> RimCustomVfpPlot::availableValues( RimVfpDefines::ProductionVariableType variableType ) const
{
    ...
    {
        std::vector<RimVfpTable*> tables = m_compareTables.ptrReferencedObjectsByType();
        for ( const auto& table : tables )
        {
            if ( !table ) continue;
...
void RimCustomVfpPlot::defineUiOrdering( QString uiConfigName, caf::PdmUiOrdering& uiOrdering )
{
    ...
    {
        auto group = uiOrdering.addNewGroup( "Compare Tables" );
        group->setCollapsedByDefault();
        group->add( &m_compareTables );
    }
...
QList<caf::PdmOptionItemInfo> RimCustomVfpPlot::calculateValueOptions( const caf::PdmFieldHandle* fieldNeedingOptions )
{
    ...
    else if ( fieldNeedingOptions == &m_compareTables )
    {
...
void RimCustomVfpPlot::fieldChangedByUi( const caf::PdmFieldHandle* changedField, const QVariant& oldValue, const QVariant& newValue )
{
    ...
    if ( changedField == &m_compareTables || changedField == &m_curveMatchingType || changedField == &m_curveValueOptions ||
