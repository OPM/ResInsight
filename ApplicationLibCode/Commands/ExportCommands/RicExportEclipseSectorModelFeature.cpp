                         ...
                         ( !isWellNameRecord && ( keywordName == "COMPSEGS" || keywordName == "WELSEGS" ) && keepSegmentRecords ) )
                    {
                        ...
                        if ( keywordName == "WELSPECS" )
                        {
                            ...
                            if ( record.size() >= 4 )
                            {
                                ...
                                filteredKeyword.addRecord( Opm::DeckRecord{ std::move( items ) } );
                            }
                        }
                        else if ( keywordName == "COMPDAT" )
                        {
                            ...
                            if ( record.size() >= 5 )
                            {
                                ...
                                auto        transformResultK1 = transformIjkToSectorCoordinates( origIjkK1,
                                                                                          exportSettings.min(),
                                                                                          exportSettings.max(),
                                                                                          exportSettings.refinement() );
                                ...
                                auto        transformResultK2 = transformIjkToSectorCoordinates( origIjkK2,
                                                                                          exportSettings.min(),
                                                                                          exportSettings.max(),
                                                                                          exportSettings.refinement() );

                                if ( !transformResultK1 || !transformResultK2 )
                                {
                                    RiaLogging::warning(
                                        QString( "Skipping completion for well %1 in COMPDAT: out of sector bounds" ).arg( wellName.c_str() ) );
                                    continue;
                                }
                        ...
                        {
                            // COMPSEGS: first record is well name, subsequent records are segment data with I, J, K
                            if ( isWellNameRecord )
                            {
                                // First record - just copy
                                filteredKeyword.addRecord( Opm::DeckRecord( record ) );
                            }
                            else if ( record.size() >= 3 )
                            {
                                // Segment record - transform I, J, K (first three items)
                                std::vector<Opm::DeckItem> items;

                                // Get I, J, K from first three items
                                ...
                                auto        transformResult = transformIjkToSectorCoordinates( origIjk,
                                                                                        exportSettings.min(),
                                                                                        exportSettings.max(),
                                                                                        exportSettings.refinement() );
                                ...
                                {
                                    RiaLogging::warning(
                                        QString( "Skipping segment for well %1 in COMPSEGS: out of sector bounds" ).arg( wellName.c_str() ) );
                                    continue;
                                }
