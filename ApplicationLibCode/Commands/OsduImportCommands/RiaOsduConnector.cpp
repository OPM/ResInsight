...
///
//--------------------------------------------------------------------------------------------------
void RiaOsduConnector::doIt( const QString& server, const QString& dataPartitionId, const QString& token )
{
    ...
    QString WELLBORE_TRAJECTORY_KIND = "osdu:wks:work-product-component--WellboreTrajectory:1.1.0";
    ...
    else if ( m_parser->isSet( "w" ) )
    {
        ...
        params["kind"]  = WELL_KIND;
        params["limit"] = "10000";
        params["query"] = QString( "nested(data.GeoContexts, (FieldID:\"%1\"))" ).arg( m_parser->value( "w" ) );
        makeRequest( params, server, dataPartitionId, token );
