...
void main() 
{
...
#if defined(USE_TEXTURE_RECT)
	avgValue += texture(u_blurSampler, gl_FragCoord.xy) * incrementalGaussian.x;
#else
    ...
    { 
#if defined(USE_TEXTURE_RECT)
        avgValue += texture(u_blurSampler, gl_FragCoord.xy - i*blurMultiplyVec) * incrementalGaussian.x;         
        avgValue += texture(u_blurSampler, gl_FragCoord.xy + i*blurMultiplyVec) * incrementalGaussian.x;         
#else
