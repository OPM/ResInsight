...
QString caf::PdmMarkdownGenerator::generate( PdmObjectFactory* factory ) const
{
    ...
    std::vector<std::shared_ptr<const PdmObject>> commandObjects;
    std::vector<std::shared_ptr<const PdmObject>> dataModelObjects;
    for ( QString classKeyword : classKeywords )
    {
        ...
        auto xml   = object->xmlCapability();
        auto stack = xml->classInheritanceStack();

        // xml->inheritsClassWithKeyword();

        std::shared_ptr<const PdmObject> sharedObject( object );
        if ( PdmObjectScriptabilityRegister::isScriptable( sharedObject.get() ) )
        {
            dataModelObjects.push_back( sharedObject );
        }
    }

    out << generateDocDataModelObjects( dataModelObjects );
    out << generateDocCommandObjects( commandObjects );
...
//--------------------------------------------------------------------------------------------------
;
QString caf::PdmMarkdownGenerator::generateDocDataModelObjects( std::vector<std::shared_ptr<const PdmObject>>& dataModelObjects ) const
{
    QString     generatedCode;
    QTextStream out( &generatedCode );
    ...
    for ( std::shared_ptr<const caf::PdmObject> object : dataModelObjects )
    {
        ...
        for ( auto it = classInheritanceStack.begin(); it != classInheritanceStack.end(); ++it )
        {
            ...
            {
                QString classCode;
                for ( size_t level = 0; level < inheritanceLevel; level++ )
                {
                    classCode += "#";
                }
                classCode += QString( " %1\n" ).arg( scriptClassName );
...
///
//--------------------------------------------------------------------------------------------------
QString caf::PdmMarkdownGenerator::generateDocCommandObjects( std::vector<std::shared_ptr<const PdmObject>>& commandObjects ) const
{
