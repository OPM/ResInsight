...
RivGeoMechPartMgr* RivGeoMechVizLogic::getUpdatedPartMgr(RivGeoMechPartMgrCache::Key pMgrKey)
{
    ...
    for (int femPartIdx = 0; femPartIdx < partCount; ++femPartIdx)
    {
        ...
        if (pMgrKey.geometryType() == RANGE_FILTERED)
        {
            ...
            RivFemElmVisibilityCalculator::computeRangeVisibility(
                elmVisibility.p(), caseData->femParts()->part(femPartIdx), cellRangeFilter);
        }
        ...
        {
            RivFemElmVisibilityCalculator::computeOverriddenCellVisibility(
                elmVisibility.p(), caseData->femParts()->part(femPartIdx), m_geomechView->viewController());
        }
