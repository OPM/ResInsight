...
#pragma omp parallel for
        for ( int elmIdx = 0; elmIdx < elementCount; ++elmIdx )
        {
            ...
            // TODO: use fempart.isHexahedron when merged
            if ( elmType == HEX8P || elmType == HEX8 )
            {
                for ( int elmNodIdx = 0; elmNodIdx < elmNodeCount; ++elmNodIdx )
                {
                    ...
                    // TODO: Make water density settable
                    double cellCenterHydroStaticPressure =
                        RigGeoMechWellLogExtractor::hydroStaticPorePressureAtDepth( cellCentroidTvdRKB, 1.03 );
