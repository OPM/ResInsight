...
TEST(RigHexIntersectionTools, DISABLED_planeHexCellIntersectionPerformanceTest)
{
    ...
    for (int run = 0; run < 5; run++)
    {
        ...
        for (int i = 0; i < 2000000; i++)
        {
            ...
            isCellIntersected = RigHexIntersectionTools::planeHexCellIntersection(
                hexCorners, fracturePlaneIntersecting, intersectionLineSegments);
            isCellIntersected = RigHexIntersectionTools::planeHexCellIntersection(
                hexCorners, fracturePlaneNotIntersecting, intersectionLineSegments);
        }
