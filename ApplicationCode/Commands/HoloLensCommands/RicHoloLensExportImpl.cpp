        ...
        for (auto& visiblePart : visibleParts)
        {
            if (RicHoloLensExportImpl::isGrid(visiblePart.p()))
            {
                ...
                if (visiblePart->effect())
                {
                    ...
                    {
                        // The logic below that inverts the culling mode simply does not make sense. We should be able to just
                        // utilize the cull mode set in the render state. The proper solution is probably to put more effort into
                        // correctly determining the winding in further up in this function, but currently there is no clear way
                        // to accomplish this.
