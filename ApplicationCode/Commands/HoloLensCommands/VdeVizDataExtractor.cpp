                                              ...
                                              VdePacketDirectory* packetDirectory)
{
        ...
        {
            {
                ...
                arrayIdsThisMesh.texCoordsArrId = m_cachingIdFactory->getOrCreateIdForFloatArr(
                    VdeCachingHashedIdFactory::TexCoordsArr, floatArr, arrElementCount);
            ...
            }
            {
                ...
                arrayIdsThisMesh.texImageArrId    = m_cachingIdFactory->getOrCreateIdForUint8Arr(
                    VdeCachingHashedIdFactory::TexImage, byteArr->ptr(), byteArr->size());
    ...
    *modelMetaJsonStr = createModelMetaJsonString(meshArr, allMeshesArrayIdsArr, labelAndPositionsArr);
    ...
    RiaLogging::debug(
        QString("HoloLens: Extracted %1 meshes (total of %2 primitives) in %3ms  (buildMeshes=%4ms, fillPacketDir=%5ms)")
            .arg(meshCount)
...
//--------------------------------------------------------------------------------------------------
QString
    VdeVizDataExtractor::createModelMetaJsonString(const std::vector<std::unique_ptr<VdeMesh>>&           meshArr,
