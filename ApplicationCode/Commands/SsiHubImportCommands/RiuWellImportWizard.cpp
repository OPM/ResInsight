...
void RiuWellImportWizard::issueHttpRequestToFile(QString completeUrlText, QString destinationFileName)
{
    ...
    if (!m_file->open(QIODevice::WriteOnly))
    {
        QMessageBox::information(
            this, tr("HTTP"), tr("Unable to save the file %1: %2.").arg(destinationFileName).arg(m_file->errorString()));
        delete m_file;
        m_file = nullptr;
        return;
    }
...
void RiuWellImportWizard::httpFinished()
{
    ...
    else if (!redirectionTarget.isNull())
    {
        ...
        if (QMessageBox::question(
                this, tr("HTTP"), tr("Redirect to %1 ?").arg(newUrl.toString()), QMessageBox::Yes | QMessageBox::No) ==
            QMessageBox::Yes)
        {
            m_url = newUrl;
            m_reply->deleteLater();
...
void RiuWellImportWizard::slotAuthenticationRequired(QNetworkReply* networkReply, QAuthenticator* authenticator)
{
    ...
    else
    {
        QMessageBox::information(this,
                                 ...
                                 "Failed to authenticate credentials. You will now be directed back to the first wizard page.");
        m_firstTimeRequestingAuthentication = true;
        ...
        while (it.hasNext())
        {
            ...
            if (m_wellPathImportObject->wellTypeSurvey)
            {
                ...
                if (!oilFieldEntry->find(name, RimWellPathEntry::WELL_SURVEY))
                {
                    ...
                    RimWellPathEntry*       surveyWellPathEntry = RimWellPathEntry::createWellPathEntry(
                        name, surveyType, requestUrl, m_destinationFolder, RimWellPathEntry::WELL_SURVEY);
            ...
            if (m_wellPathImportObject->wellTypePlans)
            {
                ...
                while (planIt.hasNext())
                {
                    ...
                    if (!oilFieldEntry->find(name, RimWellPathEntry::WELL_PLAN))
                    {
                        ...
                        RimWellPathEntry*       surveyWellPathEntry = RimWellPathEntry::createWellPathEntry(
                            name, surveyType, requestUrl, m_destinationFolder, RimWellPathEntry::WELL_PLAN);
                        oilFieldEntry->wells.push_back(surveyWellPathEntry);
                    }
