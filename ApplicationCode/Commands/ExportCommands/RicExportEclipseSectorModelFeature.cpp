...
void RicExportEclipseSectorModelFeature::openDialogAndExecuteCommand(RimEclipseView* view)
{
    ...
    RiuPropertyViewTabWidget propertyDialog(
        Riu3DMainWindowTools::mainWindowWidget(), exportSettings, "Export Eclipse Sector Model", exportSettings->tabNames());
                                                        ...
                                                        const QString&                       logPrefix)
{
    ...
    cvf::Vec3st refinement(
        exportSettings.refinementCountI(), exportSettings.refinementCountJ(), exportSettings.refinementCountK());
    ...
    if (exportSettings.exportParameters() != RicExportEclipseSectorModelUi::EXPORT_NO_RESULTS)
    {
        ...
        if (exportSettings.exportParameters == RicExportEclipseSectorModelUi::EXPORT_TO_SEPARATE_FILE_PER_RESULT)
        {
            ...
            for (QString keyword : keywords)
            {
                ...
                bool    worked   = RifEclipseInputFileTools::exportKeywords(
                    fileName, view->eclipseCase()->eclipseCaseData(), {keyword}, fileWriteMode, min, max, refinement);
                if (!worked)
                {
        ...
        else
        {
            ...
            bool worked = RifEclipseInputFileTools::exportKeywords(
                fileName, view->eclipseCase()->eclipseCaseData(), keywords, fileWriteMode, min, max, refinement);
                                                                                          ...
                                                                                          const cvf::UByteArray& cellVisibillity)
{
    ...
    cvf::Vec3i         min =
        cvf::Vec3i(int(mainGrid->cellCountI() - 1), int(mainGrid->cellCountJ() - 1), int(mainGrid->cellCountK() - 1));
