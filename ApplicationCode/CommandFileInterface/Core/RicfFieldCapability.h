...
struct RicfFieldReader<caf::AppEnum<T>>
{
                              ...
                              bool             stringsAreQuoted = true)
    {
        ...
        if (!fieldValue.setFromText(accumulatedFieldValue))
        {
            ...
            // Error message
            errorMessageContainer->addError("Argument must be valid enum value. " + errorMessageContainer->currentArgument +
                                            "\" argument of the command: \"" + errorMessageContainer->currentCommand + "\"");
        }
    }
};
...
struct RicfFieldReader<std::vector<T>>
{
                              ...
                              bool            stringsAreQuoted = true)
    {
        ...
        {
            errorMessageContainer->addError("Array argument is missing start '['. " + errorMessageContainer->currentArgument +
                                            "\" argument of the command: \"" + errorMessageContainer->currentCommand + "\"");
        }
    }
};
...
public:
                       ...
                       bool                   stringsAreQuoted = true) override
    {
        ...
        RicfFieldReader<typename FieldType::FieldDataType>::readFieldData(
            value, inputStream, errorMessageContainer, stringsAreQuoted);
