...
void RigWellLogExtractor::populateReturnArrays(std::map<RigMDCellIdxEnterLeaveKey, HexIntersectionInfo>& uniqueIntersections)
{
    ...
    std::map<RigMDEnterLeaveCellIdxKey, HexIntersectionInfo> sortedUniqueIntersections;
    {
        ...
        {
            sortedUniqueIntersections.insert(std::make_pair(
                RigMDEnterLeaveCellIdxKey(it->first.measuredDepth, it->first.isEnteringCell, it->first.hexIndex), it->second));
            ++it;
        }
    }
    ...
    // Add points for the endpoint of the wellpath, if it starts/ends inside a cell
    {
        ...
        if (it != sortedUniqueIntersections.end() &&
            !it->first.isEnteringCell) // Leaving a cell as first intersection. Well starts inside a cell.
        {
            ...
            sortedUniqueIntersections.insert(
                std::make_pair(RigMDEnterLeaveCellIdxKey(m_wellPath->m_measuredDepths[0], true, firstLeavingPoint.m_hexIndex),
                               firstLeavingPoint));
        }
        ...
        while (it1 != sortedUniqueIntersections.end())
        {
            ...
            else
            {
                    ...
                    RigMDEnterLeaveCellIdxKey::isProperCellEnterLeavePair(it11->first, it21->first))
                {
                    ...
                    {
                        RiaLogging::warning(QString("Well Log Extraction : ") + QString::fromStdString(m_wellCaseErrorMsgName) +
