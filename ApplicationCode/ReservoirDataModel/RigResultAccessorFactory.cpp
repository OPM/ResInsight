                                                                                 ...
                                                                                 RimEclipseResultDefinition* resultDefinition)
{
    ...
    else
    {
        return RigResultAccessorFactory::createFromResultAddress(
            eclipseCase, gridIndex, resultDefinition->porosityModel(), timeStepIndex, resultDefinition->eclipseResultAddress());
    }
}
                                                                                   ...
                                                                                   const RigEclipseResultAddress& resVarAddr)
{
    ...
    if (resVarAddr.m_resultName == RiaDefines::combinedTransmissibilityResultName())
    {
        ...
        cvf::ref<RigResultAccessor> xTransAccessor                = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName                    = "TRANY";
        cvf::ref<RigResultAccessor> yTransAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName                    = "TRANZ";
        cvf::ref<RigResultAccessor> zTransAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
    ...
    else if (resVarAddr.m_resultName == RiaDefines::combinedMultResultName())
    {
        ...
        cvf::ref<RigResultAccessor> multXPos = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName              = "MULTX-";
        cvf::ref<RigResultAccessor> multXNeg = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName              = "MULTY";
        cvf::ref<RigResultAccessor> multYPos = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName              = "MULTY-";
        cvf::ref<RigResultAccessor> multYNeg = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName              = "MULTZ";
        cvf::ref<RigResultAccessor> multZPos = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName              = "MULTZ-";
        cvf::ref<RigResultAccessor> multZNeg = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);

        cellFaceAccessObject->setMultResultAccessors(
            multXPos.p(), multXNeg.p(), multYPos.p(), multYNeg.p(), multZPos.p(), multZNeg.p());
    ...
    else if (resVarAddr.m_resultName == RiaDefines::combinedRiMultResultName())
    {
        ...
        cvf::ref<RigResultAccessor> xRiMultAccessor               = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName                     = RiaDefines::riMultYResultName();
        cvf::ref<RigResultAccessor> yRiMultAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName                     = RiaDefines::riMultZResultName();
        cvf::ref<RigResultAccessor> zRiMultAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
    ...
    else if (resVarAddr.m_resultName == RiaDefines::combinedRiAreaNormTranResultName())
    {
        ...
        cvf::ref<RigResultAccessor> xRiAreaNormTransAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName                              = RiaDefines::riAreaNormTranYResultName();
        cvf::ref<RigResultAccessor> yRiAreaNormTransAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName                              = RiaDefines::riAreaNormTranZResultName();
        cvf::ref<RigResultAccessor> zRiAreaNormTransAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);

        cellFaceAccessObject->setTransResultAccessors(
            xRiAreaNormTransAccessor.p(), yRiAreaNormTransAccessor.p(), zRiAreaNormTransAccessor.p());
    ...
    else if (resVarAddr.m_resultName.endsWith("IJK"))
    {
        ...
        cvf::ref<RigResultAccessor> iAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName               = QString("%1J").arg(baseName);
        cvf::ref<RigResultAccessor> jAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
        nativeAddr.m_resultName               = QString("%1K").arg(baseName);
        cvf::ref<RigResultAccessor> kAccessor = RigResultAccessorFactory::createNativeFromResultAddress(
            eclipseCase, gridIndex, porosityModel, timeStepIndex, nativeAddr);
