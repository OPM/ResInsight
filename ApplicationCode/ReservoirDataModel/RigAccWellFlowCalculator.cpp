    ...
    RigAccWellFlowCalculator::calculateAccumulatedFractions(const std::vector<double>& accumulatedFlowPrTracer) const
{
    ...
    if (totalFlow == 0.0 || !isFlowRateConsistent(totalFlow)) // If we have no accumulated flow, we set all the flow associated to
                                                              // the last tracer, which is the reservoir
    {
...
void RigAccWellFlowCalculator::calculateAccumulatedFlowPrConnection(size_t branchIdx, size_t startConnectionNumberFromTop)
{
    ...
    while (clSegIdx >= 0)
    {
        ...
        for (size_t dsBidx : downStreamBranchIndices)
        {
            ...
            if (dsBidx != branchIdx &&
                downStreamBranchFlow.depthValuesFromTop.size() == 0) // Not this branch or already calculated
            {
                                                       ...
                                                       const std::vector<double>& accBranchFlowPrTracer) const
{
    ...
    bool isAccumulationConsistent =
        isFlowRateConsistent(totalThisBranchFlow); // If inconsistent, is it always only the Reservoir tracer that has the flow ?
    ...
    std::vector<QString> tracersToGroup;
    {
        ...
        for (const auto& tracerPair : totalTracerFractions)
        {
            if (fabs(tracerPair.second) <= m_smallContributionsThreshold &&
                (hasConsistentWellFlow || tracerPair.first != RIG_RESERVOIR_TRACER_NAME)) // Do not group the Reservoir tracer if
                                                                                          // the well flow is inconsistent,
                                                                                          // because cross flow is shown as the
                                                                                          // reservoir fraction
            {
