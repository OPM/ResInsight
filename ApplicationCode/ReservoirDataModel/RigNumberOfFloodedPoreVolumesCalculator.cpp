                                                                                 ...
                                                                                 const std::vector<QString> tracerNames)
{
    ...
    porvResults = RigCaseCellResultsData::getResultIndexableStaticResult(
        actCellInfo, gridCellResults, "PORV", porvActiveCellsResultStorage);
    ...
    swcrResults                            = RigCaseCellResultsData::getResultIndexableStaticResult(
        actCellInfo, gridCellResults, "SWCR", porvActiveCellsResultStorage);
    progress.incrementProgress();
    ...
    for (size_t timeStep = 0; timeStep < daysSinceSimulationStart.size(); timeStep++)
    {
        ...
        if (hasFlowrateI)
        {
            flowrateI = &(eclipseCaseData->results(RiaDefines::MATRIX_MODEL)->cellScalarResults(flrWatIAddr, timeStep));
        }
        ...
        if (hasFlowrateJ)
        {
            flowrateJ = &(eclipseCaseData->results(RiaDefines::MATRIX_MODEL)->cellScalarResults(flrWatJAddr, timeStep));
        }
        ...
        if (hasFlowrateK)
        {
            flowrateK = &(eclipseCaseData->results(RiaDefines::MATRIX_MODEL)->cellScalarResults(flrWatKAddr, timeStep));
        }
                                                        ...
                                                        std::vector<std::vector<double>>        summedTracersAtAllTimesteps)
{
    ...
    for (size_t timeStep = 1; timeStep < daysSinceSimulationStart.size(); timeStep++)
    {
        ...
        {
            distributeNNCflow(
                connections, caseToApply, summedTracersAtAllTimesteps[timeStep - 1], flowrateNNC, totoalFlowrateIntoCell);
        }
        ...
        {
            CellQwIn[cellResultIndex] =
                cellQwInAtAllTimeSteps[timeStep - 1][cellResultIndex] + (totoalFlowrateIntoCell[cellResultIndex]) * deltaT;
        }
