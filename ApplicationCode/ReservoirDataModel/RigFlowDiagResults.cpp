                                                                   ...
                                                                   size_t                          timeStepIndex)
{
    ...
    calculateSumOfFractionAndFractionMultTOF(
        activeCellCount, injectorFractions, injectorTOFs, &injectorTotalFractions, &injectorFractMultTof);

    std::vector<double> producerTotalFractions;
    std::vector<double> producerFractMultTof;
    calculateSumOfFractionAndFractionMultTOF(
        activeCellCount, producerFractions, producerTOFs, &producerTotalFractions, &producerFractMultTof);
                                                                       ...
                                                                       size_t                          timeStepIndex)
{
    std::vector<const std::vector<double>*> fractions =
        findResultsForSelectedTracers(resVarAddr, timeStepIndex, RIG_FLD_CELL_FRACTION_RESNAME, RimFlowDiagSolution::UNDEFINED);
...
std::vector<double>* RigFlowDiagResults::calculateTracerWithMaxFractionResult(const RigFlowDiagResultAddress& resVarAddr,
                                                                              size_t                          timeStepIndex)
{
    std::vector<std::pair<std::string, const std::vector<double>*>> fractions = findNamedResultsForSelectedTracers(
        resVarAddr, timeStepIndex, RIG_FLD_CELL_FRACTION_RESNAME, RimFlowDiagSolution::UNDEFINED);
                                                      ...
                                                      RimFlowDiagSolution::TracerStatusType wantedTracerType)
{
    ...
    for (const std::string& tracerName : resVarAddr.selectedTracerNames)
    {
        ...
        {
            selectedTracersResults.push_back(findOrCalculateResult(
                RigFlowDiagResultAddress(nativeResultName, resVarAddr.phaseSelection, tracerName), timeStepIndex));
        }
    }
                                                           ...
                                                           RimFlowDiagSolution::TracerStatusType wantedTracerType)
{
    ...
    for (const std::string& tracerName : resVarAddr.selectedTracerNames)
    {
        ...
        if (tracerType != RimFlowDiagSolution::CLOSED &&
            (tracerType == wantedTracerType || wantedTracerType == RimFlowDiagSolution::UNDEFINED))
        {
            selectedTracersResults.push_back(std::make_pair(
                tracerName,
                                                   ...
                                                   int                         maxTof)
{
    ...
    if (cellSelection == CELLS_COMMUNICATION)
    {
        ...
        RigFlowDiagResultAddress communicationAddress(
            RIG_FLD_COMMUNICATION_RESNAME, RigFlowDiagResultAddress::PHASE_ALL, allTracers);
    ...
    return solverInterface()->calculateFlowCharacteristics(
        &injectorResults, &producerResults, selectedCellIndices, max_pv_fraction);
}
