...
double GeometryTools::signedAreaPlanarPolygon(const cvf::Vec3d& planeNormal, const std::vector<cvf::Vec3d>& polygon)
{
    ...
    {
        signedArea +=
            (polygon[(i + 1) % polygon.size()][X] - polygon[i][X]) * (polygon[(i + 1) % polygon.size()][Y] + polygon[i][Y]);
    }
    return signedArea;
}
...
cvf::Vec3d GeometryTools::polygonAreaNormal3D(const std::vector<cvf::Vec3d>& polygon)
{
    ...
    switch (pSize)
    {
        ...
        default:
        {
            ...
            // but I suspect this (simpler) approach is faster for small polygons, as long as we do not have the polygon normal up
            // front.
            //
