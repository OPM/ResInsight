            ...
            else
            {
                ...
                if (prevWellResPoint && prevWellResPoint->isValid())
                {
                    ...
                    distanceToWellHeadIsLonger =
                        (centerThisCell - centerPreviousCell).lengthSquared() <= (centerThisCell - whStartPos).lengthSquared();
                }
...
public:
        ...
        , m_orgWellResultFrame(awellResultFrame)
    {
        ...
        auto buildResBranchToBranchLineEndsDistMap = [&unusedBranchLineIterators, &resBranchIdxToBranchLineEndPointsDists, this](
                                                         cvf::Vec3d& fromPoint, int resultBranchIndex) {
        ...
        auto removeBranchLineFromDistanceMap = [&resBranchIdxToBranchLineEndPointsDists](
...
private:
    ...
    void addBranchLineToLastWellResultBranch(std::list<std::pair<bool, std::deque<size_t>>>::iterator branchLineIt,
                                             bool                                                     startAtFront)
    {
        addBranchLineToWellResultBranch(
            static_cast<int>(m_branchedWell.m_wellResultBranches.size()) - 1, branchLineIt, startAtFront);
    }
                                         ...
                                         bool                                                     startAtFront)
    {
        ...
        if (wellCellIndices.size())
        {
            if (!branchLineIt->first) // Is real branch, with first cell as cell *before* entry point on main branch
            {
                ...
                {
                    // Use the shared face between the cell before, and the branching cell as start point for the branch, to make
                    // the pipe "whole"

                    cvf::StructGridInterface::FaceType sharedFace = cvf::StructGridInterface::NO_FACE;
                    m_eclipseCaseData->findSharedSourceFace(
                        sharedFace, orgWellResultPoints[wellCellIndices[0]], orgWellResultPoints[wellCellIndices[1]]);
        ...
        for (size_t cIdx = 0; cIdx < cellCount; ++cIdx)
        {
            ...
            for (size_t idxToCloseCell : closeCells)
            {
                if (idxToCloseCell != cIdx && m_cellsWithNeighbors[cIdx].count(idxToCloseCell) == 0)
                {
                    ...
                    auto contactFace = RigNNCData::calculateCellFaceOverlap(
                        c1, c2, *(m_eclipseCaseData->mainGrid()), &poygonIndices, &intersections);
        ...
        for (auto& cellWithNeighborsPair : m_cellsWithNeighbors)
        {
            ...
            if (it != m_unusedWellCellIndices.end())
            {
                ...
                while (neighbour != cvf::UNDEFINED_SIZE_T)
                {
                    ...
                    {
                        m_branchLines.push_back(std::make_pair(
                            false, std::deque<size_t>{branchList.front(), cellWithNeighborsPair.first, neighbour}));
