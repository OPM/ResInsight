...
void RigNNCData::processConnections(const RigMainGrid& mainGrid)
{
    ...
    for (size_t cnIdx = 0; cnIdx < m_connections.size(); ++cnIdx)
    {
        ...
        {
            // cvf::Trace::show("NNC: No overlap found for : C1: " + cvf::String((int)m_connections[cnIdx].m_c1GlobIdx) + "C2: " +
            // cvf::String((int)m_connections[cnIdx].m_c2GlobIdx));
        }
    }
}
                                                                        ...
                                                                        std::vector<cvf::Vec3d>* connectionIntersections)
{
    ...
    if (c1.hostGrid() == c2.hostGrid())
    {
        ...
        if (!hasNeighbourInAnyDirection)
        {
            ...
            // cvf::Trace::show("NNC: No direct neighbors : C1: " + cvf::String((int)m_connections[cnIdx].m_c1GlobIdx) + " C2: " +
            // cvf::String((int)m_connections[cnIdx].m_c2GlobIdx));
