...
bool RifHdf5Reader::dynamicResult(const QString& result, size_t stepIndex, std::vector<double>* values) const
{
    ...
    try
    {
        ...
        int propIdx = (it != props.end()) ? it - props.begin()
...
QStringList RifHdf5Reader::propertyNames() const
{
    ...
    std::string fileName = m_timeStepFileNames[0]; // assume the result variables to be identical across time steps => extract
                                                   // names from first time step file
