...
RimGeoMechResultDefinition::RimGeoMechResultDefinition(void)
{
    ...
    CAF_PDM_InitField(
        &m_timeLapseBaseTimestep, "TimeLapseBaseTimeStep", RigFemResultAddress::noTimeLapseValue(), "Base Time Step", "", "", "");
    ...
    CAF_PDM_InitField(&m_compactionRefLayerUiField,
                      ...
                      "",
                      "The compaction is calculated with reference to this layer. Default layer is the topmost layer with POR",
                      "");
                                                                                ...
                                                                                bool*                      useOptionsOnly)
{
    ...
    *useOptionsOnly = true;
    ...
    if (m_geomCase)
    {
        ...
        {
            std::vector<RigFemResultPosEnum> optionItems = {
                RIG_NODAL, RIG_ELEMENT_NODAL, RIG_INTEGRATION_POINT, RIG_ELEMENT_NODAL_FACE, RIG_FORMATION_NAMES, RIG_ELEMENT};
        ...
        else if (&m_timeLapseBaseTimestep == fieldNeedingOptions)
        {
            ...
            for (size_t stepIdx = 0; stepIdx < stepNames.size(); ++stepIdx)
            {
                options.push_back(caf::PdmOptionItemInfo(
                    QString("%1 (#%2)").arg(QString::fromStdString(stepNames[stepIdx])).arg(stepIdx), static_cast<int>(stepIdx)));
            }
        }
