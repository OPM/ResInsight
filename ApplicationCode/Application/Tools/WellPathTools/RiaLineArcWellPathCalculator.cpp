                                                           ...
                                                           const std::vector<WellTarget>& activeWellPathTargets)
{
    ...
    m_targetStatuses.resize(
        activeWellPathTargets.size(),
        {false, 0.0, 0.0, false, std::numeric_limits<double>::infinity(), false, std::numeric_limits<double>::infinity()});
    ...
    if (!adjustedWellPathTargets[0].isTangentConstrained)
    {
        ...
        m_startTangent =
            RiaOffshoreSphericalCoords::unitVectorFromAziInc(target1Status.resultAzimuth, target1Status.resultInclination);
    }
    else
    {
        ...
        for (size_t tIdx = startSSegmentIdx; tIdx < endSSegementIdx; ++tIdx)
        {
            ...
            if (target1.isTangentConstrained && target2.isTangentConstrained)
            {
                ...
                if (sCurveCalc.solveStatus() != RiaSCurveCalculator::CONVERGED)
                {
                    ...
                    // RiaLogging::warning("Using fall-back calculation of well path geometry between active target number: " +
                    // QString::number(tIdx+1) + " and " + QString::number(tIdx+2));
    ...
    if (adjustedWellPathTargets.size() > 2 && endSSegementIdx < (adjustedWellPathTargets.size() - 1))
    {
        ...
        RiaJCurveCalculator jCurve(
            target1.targetPointXYZ, target1.azimuth, target1.inclination, target1.radius2, target2.targetPointXYZ);
