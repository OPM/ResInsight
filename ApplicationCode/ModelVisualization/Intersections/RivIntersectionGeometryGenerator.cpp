    ...
    if (m_isFlattened)
    {
        ...
        for (size_t pLineIdx = 0; pLineIdx < m_polyLines.size(); ++pLineIdx)
        {
            ...
            m_segementTransformPrLinePoint.emplace_back(
                RivSectionFlattner::calculateFlatteningCSsForPolyline(polyLine, m_extrusionDirection, startOffset, &startOffset));
        }
    }
    else
    {
...
#define isFace(faceEnum) (0 <= faceEnum && faceEnum <= 5)
        ...
        {
            const RigFault* fault = m_hexGrid->findFaultFromCellIndexAndCellFace(
                globalCellIdx, (FaceType)cellFaceForEachClippedTriangleEdge[triVxIdx]);
            if (fault)
            {
    ...
    for (size_t pLineIdx = 0; pLineIdx < m_polyLines.size(); ++pLineIdx)
    {
        ...
        while (lIdx < lineCount - 1)
        {
            ...
            for (size_t cccIdx = 0; cccIdx < columnCellCandidates.size(); ++cccIdx)
            {
                ...
                caf::HexGridIntersectionTools::planeHexIntersectionMC(
                    plane, cellCorners, cornerIndices, &hexPlaneCutTriangleVxes, &cellFaceForEachTriangleEdge);
                    ...
                    for (int i = 0; i < 3; ++i)
                    {
                        ...
                        if (cvx.isVxIdsNative)
                        {
                            m_triVxToCellCornerWeights.push_back(RivIntersectionVertexWeights(
                                cvx.clippedEdgeVx1Id, cvx.clippedEdgeVx2Id, cvx.normDistFromEdgeVx1));
                        }
                        else
                        {
