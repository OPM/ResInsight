        ...
        if (attribute->isEnabled())
        {
            if (attribute->componentType() == RiaDefines::CASING)
            {
                ...
                geoGenerator.tubeWithCenterLinePartsAndVariableWidth(
                    &parts, displayCoords, radii, attribute->defaultComponentColor());
                for (auto part : parts)
                {
                                                   ...
                                                   bool                              doFlatten)
{
    ...
    for (RimPerforationInterval* perforation : perforations)
    {
        ...
        {
            pair<vector<cvf::Vec3d>, vector<double>> perfintervalCoordsAndMD = wellPathGeometry->clippedPointSubset(
        ...
        {
            cvf::Vec3d         dummy;
            vector<cvf::Mat4d> flatningCSs = RivSectionFlattner::calculateFlatteningCSsForPolyline(
                perfIntervalCL, cvf::Vec3d::Z_AXIS, {horizontalLengthAlongWellPath, 0.0, perfIntervalCL[0].z()}, &dummy);
                                            ...
                                            bool                              doFlatten)
{
    ...
    if (wellPathCollection->wellPathClip)
    {
        ...
        clippedWellPathCenterLine = RigWellPath::clipPolylineStartAboveZ(
            wellpathCenterLine, maxZClipHeight, &horizontalLengthAlongWellToClipPoint, &idxToFirstVisibleSegment);
    }
    else
    {
