...
void RiuPvtPlotUpdater::updateOnSelectionChanged(const RiuSelectionItem* selectionItem)
{
    ...
    RiuEclipseSelectionItem* eclipseSelectionItem =
        dynamic_cast<RiuEclipseSelectionItem*>(const_cast<RiuSelectionItem*>(selectionItem));
    RimEclipseView* eclipseView = eclipseSelectionItem ? eclipseSelectionItem->m_view.p() : nullptr;

    if (!eclipseSelectionItem && !eclipseView)
    {
        const Riu2dIntersectionSelectionItem* intersectionSelItem =
            dynamic_cast<const Riu2dIntersectionSelectionItem*>(selectionItem);
...
void RiuPvtPlotUpdater::updateOnTimeStepChanged(Rim3dView* changedView)
{
    ...
    const RiuEclipseSelectionItem* eclipseSelectionItem =
        dynamic_cast<const RiuEclipseSelectionItem*>(Riu3dSelectionManager::instance()->selectedItem());
        ...
        if (activeCellIndex != cvf::UNDEFINED_SIZE_T)
        {
            ...
            cvf::ref<RigResultAccessor> rsAccessor =
                RigResultAccessorFactory::createFromResultAddress(eclipseCaseData,
                                                                  ...
                                                                  timeStepIndex,
                                                                  RigEclipseResultAddress(RiaDefines::DYNAMIC_NATIVE, "RS"));
            cvf::ref<RigResultAccessor> rvAccessor =
                RigResultAccessorFactory::createFromResultAddress(eclipseCaseData,
                                                                  gridIndex,
                                                                  RiaDefines::MATRIX_MODEL,
                                                                  timeStepIndex,
                                                                  RigEclipseResultAddress(RiaDefines::DYNAMIC_NATIVE, "RV"));
            cvf::ref<RigResultAccessor> pressureAccessor = RigResultAccessorFactory::createFromResultAddress(
                eclipseCaseData,
                gridIndex,
                RiaDefines::MATRIX_MODEL,
                timeStepIndex,
            ...
            cvf::ref<RigResultAccessor> pvtnumAccessor =
                RigResultAccessorFactory::createFromResultAddress(eclipseCaseData,
                                                                  ...
                                                                  RiaDefines::MATRIX_MODEL,
                                                                  timeStepIndex,
                                                                  RigEclipseResultAddress(RiaDefines::STATIC_NATIVE, "PVTNUM"));
            ...
            eclipseResultCase->flowDiagSolverInterface()->calculatePvtDynamicPropertiesFvf(
                activeCellIndex, cellValues.pressure, cellValues.rs, cellValues.rv, &fvfDynProps.bo, &fvfDynProps.bg);
