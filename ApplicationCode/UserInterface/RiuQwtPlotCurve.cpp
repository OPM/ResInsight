                                                    ...
                                                    bool                          keepOnlyPositiveValues)
{
    setSamplesFromXValuesAndYValues(
        RiuQwtPlotCurve::fromQDateTime(dateTimes), yValues, std::vector<double>(), keepOnlyPositiveValues);
}
...
{
    setSamplesFromXValuesAndYValues(
        RiuQwtPlotCurve::fromTime_t(dateTimes), yValues, std::vector<double>(), keepOnlyPositiveValues);
}
...
std::vector<double> RiuQwtPlotCurve::fromTime_t(const std::vector<time_t>& timeSteps)
{
    ...
    if (!timeSteps.empty())
    {
        ...
        {
            double milliSecSinceEpoch = time * 1000; // This is kind of hack, as the c++ standard does not state what time_t is.
                                                     // "Almost always" secs since epoch according to cppreference.com
