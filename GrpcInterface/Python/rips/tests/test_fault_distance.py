...
def test_add_fault_distance_subset(rips_instance, initialize_test):
    ...
    result = view.add_fault_distance("PY_FAULTDIST", faults[:1])
    assert result is not None
    assert result.result_name == "PY_FAULTDIST"


def test_add_fault_distance_all_matches_static(rips_instance, initialize_test):
    """Passing every fault into add_fault_distance must reproduce the
    ...
    view.add_fault_distance("PY_FAULTDIST_ALL", all_faults)
